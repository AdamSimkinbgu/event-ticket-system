...
public class OrderReceiptMapper {
     ...
     */
    public PurchaseHistoryDTO.PurchaseRecordDTO toPurchaseRecordDTO(OrderReceipt receipt, ITicketRepository ticketRepository) {
    ...
    /**
     * Maps only the selected tickets of a receipt.    // overloading of the above function
     *
     ...
     */
    public PurchaseHistoryDTO.PurchaseRecordDTO toPurchaseRecordDTO(OrderReceipt receipt, List<Ticket> selectedTickets) {   // overloading of the above function
    ...
    @Deprecated
    public PurchaseHistoryDTO.PurchaseRecordDTO OrderReceiptToPurchaseRecordDTO(OrderReceipt receipt, ITicketRepository ticketRepository) {
    ...
    // this function does the actual mapping work from  *OrderReceipt to PurchaseRecordDTO* , with options to include only selected tickets and to include/exclude transactions.
    private PurchaseHistoryDTO.PurchaseRecordDTO map(OrderReceipt receipt, List<Ticket> tickets,
        ...
        // Create a map of ticketId to Ticket for quick lookup when mapping receipt lines to ticket DTOs.
        ...
        // If selectedTicketsOnly is true, filter the receipt lines to include only those that correspond to the selected tickets. Otherwise, include all lines.
        ...
        // Calculate totalPaid based on whether we're including all tickets or only selected tickets. If only selected, sum the priceAtReservation of the included lines. Otherwise, use the receipt's total amount.
        double totalPaid = selectedTicketsOnly
                ? linesToMap.stream().mapToDouble(ReceiptLine::getPriceAtReservation).sum()
                : receipt.getTotalAmount();

        // Map the receipt lines to ticket DTOs, using the ticketsById map to enrich with current ticket status. If a ticket is missing (which shouldn't happen but we handle it gracefully), use a fallback status based on whether the receipt was refunded.
        ...
        // Optionally map the transaction records to transaction DTOs if includeTransactions is true. Otherwise, use an empty list.
    ...
    private PurchaseHistoryDTO.TicketRecordDTO toTicketRecordDTO(OrderReceipt receipt, ReceiptLine line, Map<Integer, Ticket> ticketsById) {
