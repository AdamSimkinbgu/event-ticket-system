...
public class ReservationService {
    ...
    public ReservationService(IEventRepository eventRepository, IActiveOrderRepository activeOrderRepository,
            ISessionManager iSessionManager, INotificationService notificationService) {
    ...
    // public ReservationResultDTO removeOneReservedTicket(String token, int
    // eventId, int zoneId) {
    // eventLogger.info("Entered removeOneReservedTicket function: eventId={},
    // zoneId={}",
    // eventId, zoneId);

    // try {
    // if (token == null || token.isBlank()) {
    // eventLogger.warn("removeOneReservedTicket rejected: missing authentication
    // token");
    // throw new IllegalArgumentException("Missing authentication token");
    // }

    // if (!iSessionManager.validateToken(token)) {
    // eventLogger.warn("removeOneReservedTicket rejected: invalid or expired
    // token");
    // throw new IllegalStateException("Invalid or expired authentication token");
    // }
    ...
    // if (userId <= 0) {
    // eventLogger.warn("removeOneReservedTicket rejected: invalid buyer id={}",
    // userId);
    // throw new IllegalArgumentException("Invalid buyer id");
    // }
    ...
    // Event event = eventRepository.findById(eventId);
    // if (event == null) {
    // notificationService.notifyRemoveTicketReservationFailure(userId, eventId,
    // zoneId,
    // "Remove reservation failed: event not found");
    // eventLogger.warn("removeOneReservedTicket rejected: event not found.
    // eventId={}", eventId);
    // throw new IllegalArgumentException("Event not found: " + eventId);
    // }
    ...
    // eventLogger.warn("removeOneReservedTicket rejected: zone not found.
    // eventId={}, zoneId={}",
    // eventId, zoneId);
    // throw new IllegalArgumentException("Zone not found: " + zoneId);
    // }
    ...
    // eventLogger.warn("removeOneReservedTicket rejected: active order not found.
    // userId={}", userId);
    ...
    // eventLogger.info("removeOneReservedTicket completed successfully: userId={},
    // eventId={}, zoneId={}",
            ...
            synchronized (activeOrder) {
                synchronized (zone) {
                    ...
                    if (!activeOrder.hasReservationForEvent(eventId)) {
                        eventLogger.warn(
                                "removeOneReservedTicket rejected: active order does not contain event. userId={}, eventId={}",
                                userId, eventId);
                    ...
                    if (!activeOrder.hasTicket(eventId, zoneId)) {
                        eventLogger.warn(
                                "removeOneReservedTicket rejected: no reserved ticket to remove. userId={}, eventId={}, zoneId={}",
                                userId, eventId, zoneId);
